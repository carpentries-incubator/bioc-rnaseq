...
```{r, fig.width = 5*2, fig.height = 5*2, dpi = 72*2}
library(gplots)
...
set. On the other hand, it is quite expected because the two events are
actually biologically relevant where one is from a comparison between genders
and the other is the set of gender-related genes.
...
```{r, echo = FALSE, fig.height = 9, fig.width = 12, out.width = "80%"}
...
ComplexHeatmap:::grid.text(ComplexHeatmap:::gt_render("In total _n_ genes"), x = 0.1, y = 0.9, just = "left", gp = gpar(fontsize = 16))
ComplexHeatmap:::grid.text(ComplexHeatmap:::gt_render("Pick _n_<sub>+1</sub> XY genes"), x = 0.6, y = 0.9, just = "left", gp = gpar(fontsize = 16))
grid.lines(c(0.52, 0.59), c(0.8, 0.89))
grid.circle(0.1, 0.05, 0.02, gp = gpar(fill = "red"))
ComplexHeatmap:::grid.text(ComplexHeatmap:::gt_render("_n_<sub>1+</sub> DE genes"), 0.13, 0.05, just = "left", gp = gpar(fontsize = 16))
grid.circle(0.1+0.2, 0.05, 0.02, gp = gpar(fill = "blue"))
ComplexHeatmap:::grid.text(ComplexHeatmap:::gt_render("_n_<sub>2+</sub> non-DE genes"), 0.13+0.2, 0.05, just = "left", gp = gpar(fontsize = 16))
```
...
We have learnt the basic method for performing ORA analysis. Now we go to the
second component of the analysis: the gene sets.
...
significantly more cell cycle genes are differentially expressed than
...
Gene Ontology (GO) is the standard source for gene set enrichment analysis.
The associations between GO terms and genes are integrated in the Bioconductor
standard packages: _the oragnism annotation packages_. In current Bioconductor
release 3.17, there are the following organism packages:
...
database object _org.Hs.eg.db_ in the `OrgDb` class. The object contains a
connection to a local SQLite database. Users can simply think _org.Hs.eg.db_
...
it from the _org.Hs.eg.db_ object.
...
To get the GO gene set, we first obtain all GO IDs under the BP (biological
...
a vector of GO IDs, select ENTREZIDs which correspond every one of them_".
...
specific offspring terms in the database. Upstream merging should be done by
tools which perform analysis. In this way, the mapping between `"GO"` and
`"ENTREZID"` only contains "primary" annotations which is not complete, and
mapping between "GOALL" and "ENTREZID" is the correct one to use.
...
consider to look for with the **AnnotationHub** package, which additionally
provide the `OrgDb` objects for approximately 2000 organisms. The `OrgDb`
...
[KEGG pathway](https://www.genome.jp/kegg/pathway.html) provides its data via
a REST API (https://rest.kegg.jp/). There are several commands to retrieve
specific types of data. To retrieve the pathway gene sets, we can use the
"link" command as shown in the following URL ("link" means to link genes to
pathways). When you enter the URL in the web brower:
...
Instead directly reading from the URLs, there are also R packages which
help to obtain data from the KEGG database, such as the **KEGGREST** package
or the `download_KEGG()` function from the **clusterProfiler** package. 
...
to symbols in advances; or 2. Simply specify the ID type of the DE genes and
...
- `universe`: the unverse set of genes, i.e. total genes to use. By default it
...
for downstream anlaysis. On the other hand, the meaning of `pvalueCutoff` is
not precise and there is redundancy between `pvalueCutoff` and `pvalueCutoff`
(adjusted _p_-values and _q_-values are always not smaller than the raw
_p_-values). Thus it is suggested to set both `pvalueCutoff` and
`qvalueCutoff` to 1.
...
1. Make sure the DE genes are in Entrez IDs.
...
be the same as in the DE genes, so we choose the `"gene_symbol"` column.
...
Implementing ORA is rather simple. The following code performs ORA on a list of gene sets.
...
intersected to the universe. However, in general the universe affects four
...
definite attribute of being either DE or non-DE and each gene has a definite
attribute of either belonging to the gene set or not. If a larger universe is
used, such as total genes where there are genes not measured nor will never
annotated to gene sets (let's call them _non-informative genes_, e.g.
non-proteining coding genes or not-expressed genes), all the non-informative
genes are implicitly assigned with an attribute of being non-DE or not in the
gene set. This implicit assignment is not proper because these genes provide
no information and they should not be included in the analysis. Adding them to
the analysis increases $n_{22}$, $n_{02}$ or $n_{20}$, makes the observation
$n_{11}$ getting further away from the null distribution, eventually generates
a smaller _p_-value. Similar reason, small universes generate large
_p_-values.
...
the `ora()` function which we have implemented and using three different
universe settings.
...
plot(df1$p_value, df2$p_value, col = 2, pch = 16, 
    ...
    xlab = "all hallmark genes (p-values)", ylab = "p-values",
    main = "compare universes")
points(df1$p_value, df3$p_value, col = 4, pch = 16)
abline(a = 0, b = 1, lty = 2)
legend("topleft", legend = c("all protein-coding genes", "all gene symbols"), 
    pch = 16, col = c(2, 4))
```

It is very straightforward to see, with a larger universe, there are more
significant gene sets, which may produce potentially more falst positives.
...
`barplot()` and `dotplot()` generates plots for a small number of significant gene sets.
...
questions are _what is the key message a plot transfer to readers_ and _what
is the major graphical element_. In the barplot or dotplot, the major
graphical element which readers may notice the easiest is the height of bars
or the offset of dots to the origin. The most message of the ORA anaysis is
the enrichment. The two examples from `barplot()` and `dotplot()` actually
fail to transfer such information to readers. In the first barplot where
`"Count"` is used as values on x-axis, the numer of DE genes in gene sets is
not a good measure of the enrichment because it has a positive relation to the
size of gene sets. A high value of `"Count"` does not mean it is more enriched.
...
gene set. The dotplot puts multiple gene sets in the same plot and the aim is
to compare between gene sets, thus gene sets should be "scaled" to make them
comparible. `"GeneRatio"` is not scaled for different gene sets and it still
has a positive relation to the gene set size, which can be observed in the
dotplot where higher the gene ratio, larger the dot size). Actually
"GeneRatio" has the same effect as "Count" (GeneRatio = Count/Total_DE_Genes),
so as has been explained in the previous paragraph, `"GeneRatio"` is not a good
measure for enrichment either.
...
$$ \log_2(\mathrm{Fold\_enrichment}) = \frac{n_{11}/n_{10}}{n_{01}/n} = \frac{n_{11}n}{n_{10}n_{01}} $$

which is the log2 of the ratio of _DE% in the gene set_ and _DE% in the universe_.
...
where $\mu$ and $\sigma$ are [the mean and standard deviation of the hypergeometric distribution](https://en.wikipedia.org/wiki/Hypergeometric_distribution).
...
In the next example, we use _z_-score as the primary variable to map to the offset
to origin, `DE_Ratio` and `Count` to map to dot colors and sizes. 
...
adjusted _p_-values. So it is actually similar here where we use log2 fold
...
reach a small _p_-value with a small DE_ratio, while a small gene set needs to
have a large DE ratio to be significant.


It is also common that we perform ORA analysis on up-regulated genes and
down-regulated separately. And we want to combine the significant terms from
the two ORA analysis in one plot. In the following code, we generate
...
It is often that GO enrichment returns a long list of significant GO terms
(e.g. several hundreds). This makes it difficult to summarize the common
functions from the long list. The last package we will introduce is the
**simplifyEnrichment** package which puts GO terms into clusters and
summarizes their common functions via word clouds.

The input of the `simplifyGO()` is a vector of GO IDs. It is recommended
to have at least 100 GO IDs for visualization.
